...
export interface TokenSurgeRow {
  service?: string;
  service_id?: string;
  current?: number;
  avg?: number;
...
/** Raw inputs to every detector — one field per underlying query's result. */
export interface AnomalyInputs {
  latencyRecords: LatencyRow[];
  costRow?: CostRow;
  tokenSurgeRecords: TokenSurgeRow[];
  runawayRecords: RunawayRow[];
  growthRecords: GrowthRow[];
  mismatchRecords: MismatchRow[];
  truncationRow?: RatioRow;
  rateLimitRow?: RatioRow;
  ttftRow?: RatioRow;
  /** Whether the tenant emits a TTFT attribute at all (capability-gated). */
  ttftPresent: boolean;
  samplingRatio: number;
}

/**
 * Pure fold: every detector's raw rows into the ranked `Anomaly[]` list.
 * Exported so Demo Mode can fold small "raw record" fixtures through this
 * SAME function real data flows through (see `demoData.ts` →
 * `DEMO_ANOMALIES`) instead of hand-typing finding objects.
 */
export const computeAnomalies = (inputs: AnomalyInputs): Anomaly[] => {
  const {
    latencyRecords,
    costRow,
    tokenSurgeRecords,
  ...
  const fleetBaselineMs = median(
    latencyRecords.map((r) => num(r.service_p95_ms)).filter((v) => v > 0),
  );
  ...
  for (const r of tokenSurgeRecords) {
  ...
  const truncRatio = num(truncationRow?.ratio);
  if (truncationRow && truncRatio > THRESHOLDS.truncationRatio) {
    anomalies.push({
      id: "truncation",
      type: "truncation",
  ...
  const rlRatio = num(rateLimitRow?.ratio);
  if (rateLimitRow && num(rateLimitRow.rate_limited) > 0 && rlRatio > THRESHOLDS.rateLimitRatio) {
    anomalies.push({
      id: "rate-limit",
      type: "rate-limit",
  ...
  if (ttftPresent) {
    const tRatio = num(ttftRow?.ratio);
    if (ttftRow && tRatio > THRESHOLDS.ttftDegradationRatio) {
      anomalies.push({
        id: "ttft-degradation",
        ...
        context: `Latest-hour time-to-first-token vs rolling average (${fmtMs(num(ttftRow.avg))})`,
        detail:
  ...
  anomalies.sort((a, b) => SEVERITY_RANK[b.severity] - SEVERITY_RANK[a.severity]);
  return anomalies;
};
 ...
 * TopFindingsCard. Pulse's own SummaryTilesRow and TopFindingsStrip never
 * pass it, so their behavior is unchanged.
 */
export const useAnomalies = (showExample = false): UseAnomaliesResult => {
  ...
  const ttft = useScopedDql<RatioRow>(
    canQuery && ttftPresent
      ...
      : "",
    { enabled: canQuery && ttftPresent && !showExample, staleTime: 60_000 },
  );

  return useMemo<UseAnomaliesResult>(() => {
    if (showExample) {
      return { anomalies: DEMO_ANOMALIES, isLoading: false, error: undefined };
    }
    const anomalies = computeAnomalies({
      latencyRecords: latency.data?.records ?? [],
      costRow: cost.data?.records?.[0],
      tokenSurgeRecords: tokenSurge.data?.records ?? [],
      runawayRecords: runaway.data?.records ?? [],
      growthRecords: growth.data?.records ?? [],
      mismatchRecords: mismatch.data?.records ?? [],
      truncationRow: truncation.data?.records?.[0],
      rateLimitRow: rateLimit.data?.records?.[0],
      ttftRow: ttft.data?.records?.[0],
      ttftPresent,
      samplingRatio,
    });
