...
- **Scan-limit selector** — query budget for the whole app; every hook routes through `useScopedDql`, which **injects** the selected `scanLimitGBytes` into every query (and injects segments + global filters). No query hardcodes a scan limit, so the selector is the single source of truth and is honored everywhere — there is no per-query opt-out.
...
Current coverage: 191 tests across 15 files spanning detection, the attribute-capability registry + enrichment-tier gating, the cache-aware cost model (cache tiers, provider accounting, blended fallback), the AI-layer model + use-case-lens drift guards, the within-trace token-growth detector, the high-frequency-tool predicate, scan-limit injection, SLA scoring, and FinOps scoring.
