...
- **Side panels & table** — Top spenders and provider mix; a sortable table with request volume, in/out tokens, context utilization, throughput (tok/s), avg/P95/P99 latency, error and timeout rates, cost, and $/1M tokens.
- **FinOps sections** *(collapsible, below the table)* — daily cost stacked bar + cost-concentration treemap; the **Model A/B swap comparison** (use-case profile + two models + five-dimension weighted score → verdict and estimated monthly saving); **cost efficiency by service**; the auto-appearing **prompt cache & reported cost** panel (`gen_ai.usage.cached_tokens` / `cache_creation_input_tokens` / `cost`); and a **session / user cost rollup** (effective cost + billable tokens per session and user), capability-gated on `session.id` + `gen_ai.user` (example data available via the Tweaks toggle when absent).
...
- **Global attribute filter & click-to-filter** — Click almost any value (model, agent, provider, service, tool, trace/span ID, k8s attribute…) to add it as an app-wide filter; high-cardinality keys (IDs) use debounced, on-demand server-side value discovery instead of pre-fetching. The filter is **trace-scoped**: a single resolver query finds every `trace.id` whose trace satisfies *all* active conditions (each condition matched by any span in the trace; conditions AND across the trace, values OR within), and every page query is then scoped to those traces via `in(trace.id, …)`. This is what makes multi-condition, cross-span filters work — e.g. *"agent A using model gpt-4o"*, where the agent name and the model live on different spans of the same trace and no per-span filter could ever match both. (`trace.id` is a `uid` column, so injected ids are wrapped in `toUid(…)`.) The match count and an "approximate" flag surface next to the filter chips.
...
Current coverage: 209 tests across 18 files spanning detection, the attribute-capability registry + enrichment-tier gating, the cache-aware cost model (cache tiers, provider accounting, blended fallback), the AI-layer model + use-case-lens drift guards, the within-trace token-growth detector, the high-frequency-tool predicate, scan-limit injection, the **trace-scoped global filter** (resolver query + `toUid` trace-id injection), the **Prompts server-side agent join + status-tile content relaxation**, **trace-topology cycle safety** (recursive agent/MCP traces collapse to cyclic node graphs — the layout must not loop), SLA scoring, and FinOps scoring.
