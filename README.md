...
## What's new in v0.2.59 — Demo Mode, AI Telemetry Audit & a second Bedrock sync

Three pieces landed together: an app-wide **Demo Mode**, a renamed **AI Telemetry Audit** tab that now also covers Bedrock's raw telemetry, and a second sync pass from the standalone Bedrock app (plus the timeframe bug that motivated it).

- **Demo Mode** — A new Tweaks toggle, distinct from the existing per-capability "Show with example data," that forces *every* wired page — Summary, Pulse, Explorer, Agents, Models, Prompts, and AWS Bedrock — to render a bundled, internally-consistent example dataset instead of querying Grail, regardless of whether real telemetry exists. Built for demos, screenshots, and trial installs in a tenant with no relevant data yet; a persistent banner in the header names it and turns off in one click.
- **AI Telemetry Audit** *(renamed from Attributes)* — The Audit tab now runs two audits side by side: the existing OpenTelemetry/OpenLLMetry span-attribute coverage check, and a new **AWS Bedrock raw-telemetry audit** covering CloudWatch `ModelInvocationLog` fields, `cloud.aws.bedrock*` metrics, and Bedrock CloudTrail event fields — none of which are `gen_ai.*` span attributes the first audit could see.
- **Bedrock timeframe-probe fix** — The tab's data-availability probe was hardcoded to a 24h lookback regardless of the selected global timeframe, so a tenant with only older data (or a shorter selected window) could read as empty. It now probes the timeframe actually in scope.
- **Bedrock — second standalone-app sync** — **AWS account names** everywhere an account id renders (Account picker, governance tables), resolved from the AWS extension's monitoring-configuration API and falling back to the bare id on any lookup miss; **KPI sparklines** for cost, error rate, and agent sessions on the runtime hero row; a new **StackedBarChart** powering the governance detail modals; a **BedrockHero** redesign; and pricing-table additions (Claude Sonnet 5, Fable 5, Titan Text Express, Nova Premier).

## Previously — v0.2.56 (AWS Bedrock)

A dedicated **AWS Bedrock** tab (under *Analyze*) for teams running models through Amazon Bedrock, built on Bedrock's own telemetry — **model-invocation logs**, **CloudWatch metrics**, and **CloudTrail** — rather than `gen_ai.*` spans. Two sub-tabs share an **Account** picker and the global timeframe: **Runtime Observability & Cost & Usage** (invocations, tokens incl. cache tiers, platform-aware cost, TPM quota headroom, latency/TTFT trend bands, agent-session table, guardrails summary, threshold findings) and **Access & Governance** (100% CloudTrail — API-call/identity/source-IP/account KPIs, clickable insight cards, a cross-region exfiltration deep-dive, and a reconciliation bridge proving the CloudTrail and ModelInvocationLog sources agree). Panels maximize to near-full-screen and tables are resizable. This release also shipped the **first stable** tag (v0.2.48): a 137-item UX/data-design overhaul (chart color system, accessibility, cause-aware empty states, scan transparency), the Pulse upstream-services detail modal, the Field Notes tab, and fixes to Smartscape upstream-caller resolution and the Explorer services table.
...
The nav is grouped into four labeled clusters: **Overview** (Summary · Pulse), **Analyze** (Explorer · Agents · Models · Prompts · AWS Bedrock), **Audit** (Telemetry), and trailing utility items (Field Notes · About).
...
- **Metric tiles** — Total agents, Invocations, Slow agents, Error rate, Est. cost, Looping agents, and **TTFT**. The TTFT tile shows the median time-to-first-token across agents that emit it (`gen_ai.response.ttft` on streamed responses, derived from the per-agent averages), falling back to a "not emitted" hint only when truly absent. Its detail modal charts per-agent TTFT and offers a **"Filter to TTFT-emitting traces"** action that scopes the whole app — via an **attribute-presence filter** — to the traces that actually contain a TTFT span (a selective filter, so it resolves reliably even where the emitting agents are low-volume).
...
- **Detail panel** with sub-tabs: **Prompts** (full content bubbles, copy / copy-all / maximize, PII banner), **Trace** (in-app span waterfall with type indicators, search, and a **span-attributes panel that groups every raw attribute by namespace** — Gen AI (`gen_ai.*`), LLM (`llm.*`), Traceloop (`traceloop.*`), OpenInference, MCP, and Dynatrace-derived GenAI topology — with humanized labels and a key/value filter, above curated Core / Error / Identifiers and a catch-all "Other attributes"; open span/trace/user-session in the platform apps), **Logs** (trace logs by severity), **Topology** (per-trace dependency graph), **Eval** (score cards), and **Info** (IDs, tokens, cost, log counts).
...
- **Hero + KPI row** — invocations, accounts, models, agent sessions, tokens (including cache-read/write), and **platform-aware cost** priced through the same cache-aware cost model, unwrapping each Bedrock invocation to its underlying vendor; cost, error-rate, and agent-session tiles each carry a **sparkline**. Account ids throughout the tab (Account picker, tables) resolve to their **friendly connection name** via the AWS extension's monitoring-configuration API, falling back to the bare id on any lookup miss.
...
### AI Telemetry Audit — instrumentation coverage

Under **Audit**, a self-serve check of whether your telemetry actually emits what the app depends on — the honest complement to the panels that light up automatically once data exists.

- **OTel span-attribute audit** — 10 categories (provider/model identity, tokens, request params, operation/agent identity, workflow/framework context, prompts, errors, RAG, feedback, evaluation…) each testing every attribute at once against live span data — honoring the selected timeframe, scan limit, sampling, segments, and global filters — and marking it present or missing. Attributes are rated **M** (Mandatory) through **O** (Other), rolling up to a "Required coverage %" hero stat. An **AI-bucket detection** step finds which Grail bucket(s) actually hold AI spans in the current timeframe and can scope the whole page to one, partition-pruning the scan.
- **AWS Bedrock raw-telemetry audit** — the same present/missing treatment for what OTel spans can't see: CloudWatch `ModelInvocationLog` fields, `cloud.aws.bedrock*` metrics, and Bedrock CloudTrail event fields. Log/event fields are scored as a **share of their section's population** (present / sparse / missing); metrics are binary (a metric either delivered a datapoint in the window or it didn't). Deliberately tenant-wide — only the global timeframe applies, no segment/filter/account scoping — since a coverage audit needs to see the whole fleet or it risks reporting a false gap. `Optional` fields (Guardrails, TPM-quota / log-delivery meta-metrics) never count against the coverage stat or render as a red/critical verdict — their absence is a healthy default, not a gap.
...
- **Demo Mode** — A separate, coarser Tweaks toggle that forces *every* wired page to render its bundled example dataset instead of querying Grail, regardless of whether real telemetry exists — for demos, screenshots, and trial installs in a tenant with no relevant data yet. A persistent header banner names it and turns it off in one click; unlike "Show with example data," it doesn't wait for a capability gap.
...
- **About page** — A standard attribution page (nav: **About**) with author/maintainer/repo/license, the requested Grail scopes, and **build metadata** (version + git commit hash + commit date), injected at build time by `scripts/build-info.mjs` (a prebuild/predeploy hook that regenerates a git-ignored module the UI imports).
...
Current coverage: 943 tests across 97 files spanning detection, the attribute-capability registry + enrichment-tier gating, the cache-aware cost model (cache tiers, provider accounting, blended fallback), the AI-layer model + use-case-lens drift guards, the within-trace token-growth detector, the high-frequency-tool predicate, scan-limit injection, the **hybrid global filter** (direct per-span injection + the cross-span trace-scope resolver, including the **250-sub-expressions-per-expression cap safety** so a large trace set can't overflow DQL, and the **attribute-presence "exists" condition**), the **fleet TTFT summary** (per-agent → median/P90 + emitting-agent set), the **trace span-attribute namespace grouping** (humanized keys, per-namespace sections, "Other" catch-all), the **Prompts focus presets** (every same-span predicate + cross-span trace resolver) and the **server-side agent join + status-tile content relaxation**, **trace-topology cycle safety** (recursive agent/MCP traces collapse to cyclic node graphs — the layout must not loop), SLA scoring, FinOps comparison scoring, the **upstream-services detail** (query builders, graph assembly, sort/top-N, Smartscape URL builder), the **semantic status-color** helpers, the **AWS Bedrock** tab (ModelInvocationLog parsing, platform-aware cost, metric/runtime query builders, agent-session performance, cross-region exfiltration/residency analysis, and the insight/geometry helpers), the **AWS raw-telemetry audit catalog + verdict classification**, and each page's bundled **Demo Mode** dataset.

## Known issues

- **Maximized trace span-attributes panel won't scroll** *(to be addressed in a future build)* — In the Prompts → Trace "Distributed trace" modal, maximizing the span-attributes panel and expanding enough sections to overflow the viewport leaves the lower attributes unreachable (the wheel falls through to the page). Root cause is a flexbox `min-height: auto` interaction that prevents the list's `max-height` from clamping, so it never becomes a scroll container; adding `min-height: 0` to the scroll div alone did not resolve it and the fix likely needs the same treatment across the flex-ancestor chain (or a single bounded scroll wrapper at the modal level). Non-maximized (side-panel) attribute viewing is unaffected. Tracked for a follow-up build.
